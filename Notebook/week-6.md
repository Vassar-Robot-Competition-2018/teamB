...
### Entry by Shihan:
I wrote the codes for milestone 4 and 6, which follow very similar logics as milestone 3 and 5 respectively. The difference between 3 and 4 was really just finding some way to determine if the robot is holding a color block (if holding, drive straight until it pushes the block out of bounds). We considered using a bump sensor or IR sensor, but figured the pixy camera would work for the purpose just fine -- if the size of the color block was big enough, the robot would know it's holding a block. A global variable was used to record whether the robot is currently holding a block; this variable would be reset every time the block size became large enough and when the RGB sensed white boundary. The tricky part of milestone 4 was to make pixy camera distinguish between color boundary and color blocks. We used a makeshift strategy of pointing the camera slightly upwards so it didn't see the color boundary as much as it saw the color blocks, we also purposefully chose the color "green" out of 4 colors because we noticed it was much easier to work with green than the other three colors, for which pixy camera was overly sensitive and would almost always detect the color boundaries in stead of the color blocks. Milestone 6 was simpler; we just programmed the robot to retract and turn 90 degrees every time it hit the white boundary and go straight by default; therefore eventually the robot would travel all 4 quadrants; a global counter was constantly updated by inputs from RGB sensor to determine the quadrant.
We expect to add rotary encoders to our robot to record the distances traveled by each wheel -- we will use this information to help our robot determine its relative position to its starting point.
We were also having some problems with our lego robot body because the servos were duck-taped to the robot chassis; when the tapes wore off the the chassis would collapse inwards and would start to drag. We'll have to shift to 3D-printed body eventually.
